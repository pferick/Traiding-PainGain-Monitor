...
#define RSI_HIGH_LEVEL_LINE "RsiHighLevelLine"
#define RSI_LOW_LEVEL_LINE "RsiLowLevelLine"
#define RSI_SECOND_HIGH_LEVEL_LINE "RsiSecondHighLevelLine"
#define RSI_SECOND_LOW_LEVEL_LINE "RsiSecondLowLevelLine"
#define RSI_MID_LEVEL_LINE "RsiMidLevelLine"
#define RSI_TOP_LABEL "RsiTopLabel"
#define RSI_BOTTOM_LABEL "RsiBottomLabel"
#define RSI_MID_LABEL "RsiMidLabel"
#define RSI_MID_TOP_LABEL "RsiMidTopLabel"
#define RSI_MID_BOTTOM_LABEL "RsiMidBottomLabel"
#define RSI_UPPER_QUARTILE_LINE "RsiUpperQuartileLine"
#define RSI_LOWER_QUARTILE_LINE "RsiLowerQuartileLine"
#define RSI_REACT_TOP_LABEL "RsiReactTopLabel"
#define RSI_REACT_BOTTOM_LABEL "RsiReactBottomLabel"
#define RSI_LOW_TOP_LINE "RsiLowTopLine"
#define RSI_LOW_BOTTOM_LINE "RsiLowBottomLine"
#define RSI_LOW_TOP_LABEL "RsiLowTopLabel"
#define RSI_LOW_BOTTOM_LABEL "RsiLowBottomLabel"
...
void DeleteAllRsiObjects()
{
   ...
   ObjectDelete(0, RSI_TOP_LABEL);
   ObjectDelete(0, RSI_BOTTOM_LABEL);
   ObjectDelete(0, RSI_MID_LABEL);
   ObjectDelete(0, RSI_MID_TOP_LABEL);
   ObjectDelete(0, RSI_MID_BOTTOM_LABEL);
   ObjectDelete(0, RSI_UPPER_QUARTILE_LINE);
   ObjectDelete(0, RSI_LOWER_QUARTILE_LINE);
   ObjectDelete(0, RSI_REACT_TOP_LABEL);
   ObjectDelete(0, RSI_REACT_BOTTOM_LABEL);
   ObjectDelete(0, RSI_LOW_TOP_LINE);
   ObjectDelete(0, RSI_LOW_BOTTOM_LINE);
   ObjectDelete(0, RSI_LOW_TOP_LABEL);
   ObjectDelete(0, RSI_LOW_BOTTOM_LABEL);
}
...
void AnalyzeAndDrawRsiLevels()
{
   ...
   {
      if(rsi_values[i] <= 0 || rsi_values[i] >= 100) continue;
      if(!first_valid_found) { highest_rsi = rsi_values[i]; lowest_rsi = rsi_values[i]; first_valid_found = true; }
      else {
   ...
   rsi_react_top_level = (highest_rsi + rsi_mid_top_level) / 2.0;
   rsi_react_bottom_level = (lowest_rsi + rsi_mid_bottom_level) / 2.0;
   rsi_low_top_level = (rsi_mid_level + rsi_mid_top_level) / 2.0;
   rsi_low_bottom_level = (rsi_mid_level + rsi_mid_bottom_level) / 2.0;

   if(first_valid_found)
   {
      datetime label_time = TimeCurrent();

      // High Level Line (Gold, Solid) & Label
      ObjectCreate(0, RSI_HIGH_LEVEL_LINE, OBJ_HLINE, 1, 0, highest_rsi);
      ObjectSetInteger(0, RSI_HIGH_LEVEL_LINE, OBJPROP_COLOR, clrGold);
      ObjectSetInteger(0, RSI_HIGH_LEVEL_LINE, OBJPROP_WIDTH, 2);
      ObjectSetString(0, RSI_TOP_LABEL, OBJPROP_TEXT, "RSI-TOP");

      // Low Level Line (Gold, Solid) & Label
      ObjectCreate(0, RSI_LOW_LEVEL_LINE, OBJ_HLINE, 1, 0, lowest_rsi);
      ObjectSetInteger(0, RSI_LOW_LEVEL_LINE, OBJPROP_COLOR, clrGold);
      ObjectSetInteger(0, RSI_LOW_LEVEL_LINE, OBJPROP_WIDTH, 2);

      // Mid Levels ... (Dibujo simplificado para brevedad, usando lógica original)
      // Se mantiene la lógica de cálculo de niveles para las alertas
   }
}
